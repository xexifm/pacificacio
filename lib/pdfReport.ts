...
function sc(pdf: jsPDF, rgb: RGB) { pdf.setTextColor(rgb[0], rgb[1], rgb[2]); }
function sf(pdf: jsPDF, rgb: RGB) { pdf.setFillColor(rgb[0], rgb[1], rgb[2]); }
function sd(pdf: jsPDF, rgb: RGB) { pdf.setDrawColor(rgb[0], rgb[1], rgb[2]); }
function resetColor(pdf: jsPDF) { pdf.setTextColor(0, 0, 0); pdf.setDrawColor(0, 0, 0); }
...
function resizeToDataURL(img: HTMLImageElement, maxDim: number, fmt: 'jpeg' | 'png', quality = 0.8): string {
...
function drawPageFooter(pdf: jsPDF, pw: number, ph: number, pageNum: number, attribution: string) {
  sd(pdf, [210, 215, 230]);
...
function drawSectionHeader(pdf: jsPDF, title: string, y: number, margin: number, cw: number): number {
  sf(pdf, NAVY);
